...
function printSalarySlip(faculty: Faculty, month: number, year: number, result: SalaryResult) {
  const win = window.open('', '_blank', 'width=900,height=700,scrollbars=yes')
  if (!win) { alert('Please allow popups to print salary slips'); return }

  const breakdown = (result.breakdown ?? []).map((row) => `
    <tr class="${row.isDeduction ? 'ded' : ''}">
      <td>${row.isDeduction ? '− ' : ''}${row.label}</td>
      <td>${Number.isInteger(row.amount) || row.amount > 100 ? '₹' + row.amount.toLocaleString('en-IN') : row.amount.toFixed(1)}</td>
    </tr>`).join('')

  const carryHtml = result.carryForward ? `
    <div class="section-title">Hour Carry-Forward</div>
    <div class="carry">
      <div><div class="clabel">Prev Month Balance</div><div class="cval">${result.carryForward.previousMonthBalance.toFixed(1)} hrs</div></div>
      <div><div class="clabel">This Month Balance</div><div class="cval">${result.carryForward.currentMonthBalance.toFixed(1)} hrs</div></div>
      <div><div class="clabel">Combined Total</div><div class="cval">${result.carryForward.combinedTotal.toFixed(1)} hrs</div></div>
    </div>` : ''

  const alertsHtml = (result.alerts ?? []).filter((a) => a.level !== 'BLOCK').map((a) => `
    <div class="alert-row ${a.level === 'WARNING' ? 'alert-warn' : 'alert-info'}">${a.message}</div>`).join('')

  const today = new Date().toLocaleDateString('en-IN', { day: '2-digit', month: 'long', year: 'numeric' })
  const hoursRow = result.hoursLogged != null ? `<div><div class="flabel">Hours Logged</div><div class="fval">${result.hoursLogged} hrs</div></div>` : ''
  const daysRow  = result.daysWorked  != null ? `<div><div class="flabel">Days Worked</div><div class="fval">${result.daysWorked} days</div></div>` : ''

  win.document.write(`<!DOCTYPE html><html lang="en"><head><meta charset="utf-8">
<title>Salary Slip — ${faculty.name} — ${MONTHS_LONG[month-1]} ${year}</title>
<style>
...
<div class="slip">
  <div class="hdr">
    ...
    <div><div class="period-lbl">Salary Slip</div><div class="period">${MONTHS_LONG[month-1]} ${year}</div></div>
  </div>
  <div class="fbox">
    <div><div class="flabel">Faculty Name</div><div class="fval">${faculty.name}</div></div>
    <div><div class="flabel">Subject</div><div class="fval">${faculty.subject || '—'}</div></div>
    <div><div class="flabel">Pay Period</div><div class="fval">${MONTHS_LONG[month-1]} ${year}</div></div>
    <div><div class="flabel">Generated On</div><div class="fval">${today}</div></div>
    ${hoursRow}${daysRow}
  </div>
  ${alertsHtml ? `<div class="section-title">Notes</div><div class="alerts">${alertsHtml}</div>` : ''}
  ${carryHtml}
  ${breakdown ? `<div class="section-title">Pay Breakdown</div><table><tbody>${breakdown}<tr class="total-row"><td>Net Payable</td><td>₹${result.finalPayable?.toLocaleString('en-IN')}</td></tr></tbody></table>` : ''}
  <div class="footer">
              ...
              <div style={{ display: 'flex', gap: '0.75rem' }}>
                ...
                {selectedFaculty && (
                  <button
                    ...
                    onClick={() => printSalarySlip(selectedFaculty, month, year, result)}
