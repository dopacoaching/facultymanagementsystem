...
export default function ISChaptersPage() {
  ...
  const canEdit = ['ADMIN', 'IS_ACADEMICS_MANAGER', 'ACADEMICS_MANAGER', 'HR_MANAGER'].includes(role ?? '')
  ...
  useEffect(() => {
    ...
    setLoading(true)
    apiFetch<ISChapter[]>(`/integrated-school/chapters?batchId=${selectedBatch}`, { token: accessToken })
      .then(setChapters)
      .catch(console.error)
  ...
  async function handleStatusChange(id: string, status: ChapterStatus) {
    ...
    try {
      const updated = await apiFetch<ISChapter>(`/integrated-school/chapters/${id}`, {
  ...
  return (
    <div>
      ...
      <div className="page-header">
        <div>
          <h1 style={{ marginBottom: '0.125rem' }}>IS Chapter Progress</h1>
      ...
      <div className="card" style={{ marginBottom: '1rem', padding: '0.875rem 1.25rem' }}>
        <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap', alignItems: 'flex-end' }}>
          <div className="form-group" style={{ margin: 0, minWidth: 200 }}>
            <label className="label" style={{ marginBottom: '0.25rem' }}>IS Batch</label>
