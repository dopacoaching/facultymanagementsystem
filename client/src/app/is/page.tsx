...
export default function ISDashboard() {
  ...
  useEffect(() => {
    ...
    apiFetch<{ slots: DailySlot[] }>(
      `/integrated-school/timetable/daily?date=${today()}`,
      { token: accessToken }
    ).then((d) => setTodaySlots(d.slots)).catch(console.error)
    // Load IS chapters summary (no batchId filter = all)
    apiFetch<ISChapter[]>('/integrated-school/chapters', { token: accessToken })
      ...
      {chaptersTotal > 0 && (
        <div className="card" style={{ marginTop: '1.5rem' }}>
          <div className="card-header">
            ...
            <Link href="/is/chapters" style={{ fontSize: '0.8125rem', color: 'var(--color-primary)', fontWeight: 600, textDecoration: 'none' }}>
              View all →
            </Link>
          </div>
