...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { status, facultyId, notes, chapter, subject } = await req.json()
    ...
    console.error('[PATCH /api/integrated-school/timetable/:id]', err)
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 })
  }
}

/** DELETE /api/integrated-school/timetable/:id — hard delete PLANNED slot */
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    console.error('[DELETE /api/integrated-school/timetable/:id]', err)
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 })
  }
}
